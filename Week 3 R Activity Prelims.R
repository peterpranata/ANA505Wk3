...
peterTitanic<- as.data.frame(Titanic)
...
sex <- peterTitanic$Sex
age <- peterTitanic$Age
...
subset(peterTitanic, Freq>25)
#If I want to save the subset, I can do so below:
freqabove25 <- subset(peterTitanic, Freq>25)
...
ggplot(peterTitanic, aes(Survived, Freq)) +
  geom_col()
